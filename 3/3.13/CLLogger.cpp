...
CLLogger *CLLogger::GetInstance() {
    ...
    if (m_pLog == 0) {//第一次调用时创建

        try {
            m_pLog = new CLLogger;
        }
        catch (const char *) {//第一次创建log就失败的话，要把锁放开，后面可能会有线程进来重新创建。
            pthread_mutex_unlock(m_pMutexForCreatingLogger);
            return 0;
        }

        //每一个return之前都要记得unlock
        ...
        } else if (pthread_mutex_unlock(m_pMutexForCreatingLogger) != 0)
            return 0;
